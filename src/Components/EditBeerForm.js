...
function EditBeerForm(props){
  ...
  function handleEditBeerFormSubmission(event){
    ...
    props.onEditBeer({name: event.target.name.value, brand: event.target.brand.value, price: parseFloat(event.target.price.value), ABV: parseFloat(event.target.ABV.value), quantity: parseInt(event.target.quantity.value), id: beer.id})
  }
  return(
    <React.Fragment>
